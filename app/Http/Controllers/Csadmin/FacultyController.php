  ...
  public function facultyStatus($intCategoryId)
   ...
   function facultyProccess(Request $request)
    {    
        ...
        if(isset($aryPostData['faculty_new_password']) && $aryPostData['faculty_new_password']!=''){
   ...
   public function viewFaculty($intfacultyId=0)
  {
    ...
    public function facultyrole(Request $request, $intid=0)
    {
        ...
        if(session()->has('FILTER_ROLE'))
        {
            ...
            $resroleData = CsFacultyRole::where('role_name', 'LIKE', "%{$strFilterKeyword}%")
                        ->where('role_ins_id','=',$userId)
                        ->paginate(20);
        }else{
            $resroleData = CsFacultyRole::where('role_ins_id','=',$userId)->paginate(20);
        }    
        $resfacroleData ='';
        if($intid>0){
