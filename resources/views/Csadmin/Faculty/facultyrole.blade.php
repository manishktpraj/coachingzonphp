...
<form method="post" action="{{route('roleproccess')}}" enctype="multipart/form-data">
@csrf
...
<div class="card-body">
@include('Csadmin.bulkaction', ['status' => 'FILTER_ROLE'])
...
<td> <?php if($role->role_status==1){?>
    <a href="{{route('roleStatus',$role->role_id)}}" onclick="return confirm('Are you sure?')"><span class="badge badge-success">Active</span></a>
    <?php }else{?>
    <a href="{{route('roleStatus',$role->role_id)}}" onclick="return confirm('Are you sure?')"><span class="badge badge-danger">Inactive</span></a>
    <?php }?></td>
<td scope="col" style="text-align:center; width:100px"><div class="d-flex align-self-center justify-content-center">
<nav class="nav nav-icon-only">
<a href="{{route('permission',$role->role_id )}}" class="btn btn-info btn-icon mg-r-5" title="Permission" style="padding:0px 5px;"><i class="fas fa-copy" style="font-size:11px;"></i></a>
<a href="{{route('faculty-role',$role->role_id )}}" class="btn btn-primary btn-icon mg-r-5" title="Edit" style="padding:0px 5px;"><i class="fas fa-pencil-alt" style="font-size:11px;"></i></a>
</nav>
</div></td>
</tr>
<?php }}else{?>
