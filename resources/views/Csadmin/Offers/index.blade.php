...
<a href="{{route('add-new-offers')}}" class="btn btn-sm pd-x-15 btn-primary btn-uppercase  mg-l-5"><i data-feather="plus" class="wd-10 mg-r-5"></i>Add New Offers & Promos</a>
</div>
</div>
...
<div class="card-body">
@include('Csadmin.bulkaction', ['status' => 'FILTER_OFFER'])
</div>
...
<td> <?php if($offer->offers_status==1){?>
    <a href="{{route('offersStatus',$offer->offers_id)}}" onclick="return confirm('Are you sure?')"><span class="badge badge-success">Active</span></a>
    <?php }else{?>
    <a href="{{route('offersStatus',$offer->offers_id)}}" onclick="return confirm('Are you sure?')"><span class="badge badge-danger">Inactive</span></a>
    <?php }?></td>
<td>
<div class="d-flex align-self-center justify-content-center">
<nav class="nav nav-icon-only">
<a href="{{route('add-new-offers',$offer->offers_id )}}" class="btn btn-primary btn-icon mg-r-5" title="Edit" style="padding:0px 5px;"><i class="fas fa-pencil-alt" style="font-size:11px;"></i></a>
<a href="{{route('deleteoffer',$offer->offers_id )}}" onclick="return confirm('Are you sure?')" class="btn btn-danger btn-icon mg-r-5" title="Delete" style="padding:0px 5px;"><i class="fas fa-trash-alt" style="font-size:11px;"></i></a>
</nav>
</div>
</td>
</tr>
...
<ul class="pagination pagination-filled mg-b-0">{{ $resOfferLists->links() }}</ul>
</div>
</div>
</div>
</div>
