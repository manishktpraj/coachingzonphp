...
<a href="{{route('add-new-product')}}" class="btn btn-sm pd-x-15 btn-primary btn-uppercase  mg-l-5"><i data-feather="plus" class="wd-10 mg-r-5"></i>Add New Product</a>
</div>
</div>
...
<div class="card-body">
@include('Csadmin.bulkaction', ['status' => 'FILTER_PRODUCT'])
</div>
...
<div class="avatar"><img src="<?php echo (isset($package->product_image) && $package->product_image!="")?SITE_UPLOAD_URL.SITE_PRODUCT_IMAGE.$package->product_image:SITE_NO_IMAGE_PATH;?>" class="rounded" alt="" style="border:1px solid #ddd;"></div>
...
<td> <?php if($package->product_status==1){?>
    <a href="{{route('productStatus',$package->product_id)}}" onclick="return confirm('Are you sure?')"><span class="badge badge-success">Active</span></a>
    <?php }else{?>
    <a href="{{route('productStatus',$package->product_id)}}" onclick="return confirm('Are you sure?')"><span class="badge badge-danger">Inactive</span></a>
    <?php }?></td>
<td><?php echo date("d M Y",strtotime($package->created_at));?></td>
<td>
<div class="d-flex align-self-center justify-content-center">
<nav class="nav nav-icon-only">
<a href="{{route('add-new-product',$package->product_id )}}" class="btn btn-primary btn-icon mg-r-5" title="Edit" style="padding:0px 5px;"><i class="fas fa-pencil-alt" style="font-size:11px;"></i></a>
<a href="{{route('productDelete',$package->product_id )}}" onclick="return confirm('Are you sure?')" class="btn btn-danger btn-icon mg-r-5" title="Delete" style="padding:0px 5px;"><i class="fas fa-trash-alt" style="font-size:11px;"></i></a>
</nav>
</div>
</td>
