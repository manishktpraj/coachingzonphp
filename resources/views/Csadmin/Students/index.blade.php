...
<a href="{{route('add-new-student')}}" class="btn btn-sm pd-x-15 btn-primary btn-uppercase  mg-l-5"><i data-feather="plus" class="wd-10 mg-r-5"></i>Add New Student</a>
</div>
</div>
...
<td> <?php if($students->student_status==1){?>
    <a href="{{route('studentStatus',$students->student_id)}}" onclick="return confirm('Are you sure?')"><span class="badge badge-success">Active</span></a>
    <?php }else{?>
    <a href="{{route('studentStatus',$students->student_id)}}" onclick="return confirm('Are you sure?')"><span class="badge badge-danger">Inactive</span></a>
    <?php }?></td>
...
<ul class="pagination pagination-filled mg-b-0">{{ $resStudentData->links() }}</ul>
</div>
</div>
</div>
</div>
